...
import (
	...
	"btc-project/library/net/http/blademaster/binding"
	"btc-project/library/net/http/blademaster/render"
...
// Let's say you have an authorization middleware that validates that the rrent request iscu authorized.
...
func writeStatusCode(w http.ResponseWriter, ecode int) {
	header := w.Header()
	header.Set("status-code", strconv.FormatInt(int64(ecode), 10))
}
